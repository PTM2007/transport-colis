...
function Suivi() {
  ...
  const statutColor = (statut) => {
    switch(statut) {
      case 'Recu_en_Chine': return '#f39c12';
      case 'En_expedition': return '#3498db';
      case 'Arrive_a_destination': return '#8e44ad';
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundImage: `url(${logo})`, backgroundSize: 'cover', backgroundPosition: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 30 }}>
      <div style={{ background: 'rgba(255,255,255,0.15)', backdropFilter: 'blur(5px)', borderRadius: 16, padding: 25, width: '90%', maxWidth: 500, textAlign: 'center' }}>
        <img src={logo} alt="logo" style={{ width: 70, borderRadius: '50%', marginBottom: 8 }} />
        <h2 style={{ color: '#f5a623', margin: 0 }}>Bienvenue a</h2>
        ...
        />
        <button onClick={chercher} style={{ width: '100%', padding: 12, background: '#f5a623', color: 'white', border: 'none', borderRadius: 8, fontSize: 16, fontWeight: 'bold' }}>
          Rechercher
        </button>
